      ...
      <nav class="flex-1 overflow-y-auto py-4 px-2 space-y-2">
        <div *ngFor="let item of menuItems" class="group">
          ...
          <div *ngIf="item.isSection && !collapsed()" class="px-4 py-3 mt-4 mb-2">
            <p class="text-xs font-bold text-neutral-500 uppercase tracking-wider">{{ item.label }}</p>
          </div>

          <!-- Menu Item with potential children -->
          <div *ngIf="!item.isSection && item.children && item.children.length > 0">
            <button (click)="toggleMenu(item)" class="w-full flex items-center justify-between px-4 py-2 text-neutral-400 hover:text-primary hover:bg-neutral-800/50 rounded transition-all duration-200 group-hover:text-primary">
              <div class="flex items-center space-x-3 min-w-0">
                <svg *ngIf="item.icon" class="w-5 h-5 flex-shrink-0" [innerHTML]="getSafeIcon(item.icon)" fill="currentColor"></svg>
          ...
          <!-- Simple Menu Item -->
          <a *ngIf="!item.isSection && (!item.children || item.children.length === 0)" [routerLink]="item.route" routerLinkActive="text-primary bg-neutral-800/50" class="flex items-center space-x-3 px-4 py-2 text-neutral-400 hover:text-primary hover:bg-neutral-800/50 rounded transition-all duration-200">
            <svg *ngIf="item.icon" class="w-5 h-5 flex-shrink-0" [innerHTML]="getSafeIcon(item.icon)" fill="currentColor"></svg>
            <span *ngIf="!collapsed()" class="text-sm font-medium truncate">{{ item.label }}</span>
          </a>
        </div>
